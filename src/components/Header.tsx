...
import { Check, ChevronDown, Mail, Menu, Search, ShoppingBag, User } from "lucide-react";
...
function AnnouncementBar() {
  return (
    <div
      ...
      style={{ backgroundColor: "#4c8076", fontSize: "13px" }}
    >
      <div
        className="flex items-center justify-between"
        style={{ maxWidth: "1360px", margin: "0 auto", padding: "8px 24px", gap: "16px" }}
      >
...
function MainHeader() {
  return (
    <div
      className="w-full"
      style={{ backgroundColor: "#262f2c", height: "70px" }}
    >
      <div
        className="h-full flex items-center"
        style={{ maxWidth: "1360px", margin: "0 auto", padding: "0 16px", gap: "16px" }}
      >
        {/* Logo */}
        <a
          href="/"
          className="no-underline flex-shrink-0"
          style={{ color: "white", fontSize: "26px", fontWeight: 800, letterSpacing: "-0.5px" }}
        >
          Campsider
        </a>
        ...
        <div
          className="hidden md:flex flex-1 items-center bg-white"
          style={{ borderRadius: "4px", padding: "0 12px", gap: "8px", height: "44px" }}
        >
          <input
            type="text"
...
function CategoryNav() {
  ...
  return (
    <div
      className="hidden md:block w-full whitespace-nowrap"
      style={{
    ...
    >
      <div
        className="flex items-center h-full hide-scrollbar"
        ...
        <div className="flex items-center h-full overflow-x-auto hide-scrollbar">
          {MAIN_NAV.map((label) => (
            <a
              key={label}
              href="#"
              className="inline-flex items-center h-full hover:underline"
              style={{ color: "white", fontSize: "14px", padding: "0 14px", textDecoration: "none" }}
            >
              {label}
            </a>
          ))}
        ...
        <div className="flex items-center h-full" style={{ flex: 1, justifyContent: "flex-end" }}>
          {RIGHT_NAV.map((label) => (
            <a
              key={label}
              href="#"
              className="inline-flex items-center h-full font-bold hover:underline"
              style={{ color: "white", fontSize: "14px", padding: "0 14px", textDecoration: "none" }}
            >
              {label}
            </a>
